...
- Only `PROTOCOL_CONNECTION_LOST`, `ECONNRESET`, and `ETIMEDOUT` during the pre-DDL compatibility read use a fresh client and bounded retry; all other failures and all failures after migrator entry are never retried.
...
- `PROTOCOL_CONNECTION_LOST`, `ECONNRESET`, and `ETIMEDOUT` before migrator entry are the only retryable error codes; authentication, permission, SQL-shape, and all other errors must fail immediately.
