...
## 2026-07-14: Validate the first live query after repairing migration packaging

- tags: migration, mysql, connection-lifecycle, retry, validation
- symptom: The standalone migration runner became importable after the container packaging fix, then failed in production when MySQL closed the connection during its first compatibility query.
...
- symptom: Migration compatibility tests passed while assertions would accept a non-null `claimToken` default, a unique retry index, extra conditional DDL clauses, lowercase destructive SQL, a guard aimed at the wrong table, a missing `EXECUTE`, or a `PREPARE` sourced from the wrong variable. These drifts can exclude rows from recovery or make resumable migration steps skip/fail while the suite remains green.
