...
export interface ValidationProviderResult {
  ...
  outputValues?: readonly ValidationOutputValue[]; // 成功/失敗に加えて返される任意のキー・値ペア（value は文字列として保存）
  errorCode?: string;
  errorMessage?: string;
...
ローダー失敗は `PluginLoader.getFailedPlugins()` に記録されます。
API/Worker の `startupPlugins` は既定でこれを致命的エラーとして扱い、
プロセス起動を中止します。明示的な縮退起動が必要な環境だけ、
`VALIDATION_PLUGINS_FAIL_FAST=false` を設定すると警告ログを出して残りの
プラグインで起動します。
...
api:
  ...
  environment:
    ...
    # Optional degraded startup mode. Omit this to fail startup on plugin errors.
    # VALIDATION_PLUGINS_FAIL_FAST: "false"
  volumes:
