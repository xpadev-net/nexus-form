...
プロセスと **同一権限** で実行されます。シンボリックリンクは弾いていますが、
それ以外のサンドボックスは行いません。プラグインディレクトリへ書き込める者は
任意コード実行が可能なため、以下を運用側で担保してください。

- プラグインディレクトリはアプリ実行ユーザーから書き込み不可（root 所有・
  `:ro` マウントなど）
- 信頼できるソースから取得したプラグインのみを配置
- 配置前に内容を必ずレビュー
