...
beforeEach(() => {
  ...
  const mockIp = expectedHash("203.0.113.10", "telemetry-salt");
  mocks.selectWhere.mockReturnValue({
    orderBy: vi.fn().mockReturnValue({
      for: vi
        .fn()
        .mockResolvedValue([
          { id: "tok-1", token: "token-a", ip: mockIp, version: "V4" },
        ]),
    }),
    for: vi
      .fn()
...
describe("consumeTokensOrThrow", () => {
  it("accepts multiple submitted tokens when at least one current IP hash matches and consumes submitted live candidates", async () => {
    ...
    expect(result).toEqual([
      { version: "v4", ipHash: expectedHash("203.0.113.10", "telemetry-salt") },
    ]);
